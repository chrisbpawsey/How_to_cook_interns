...
fail when compiled due to these libraries being no longer valid or upgraded to a different version. Or one may complete their task successfully, but may 
...
deleted and thus, the user loses their work. This is a major problem for the users, especially researchers and scientist since it takes very long time 
to collect their data and vital for their researches. It is also very important that these are stored correctly within these sources.
...
Magnus is described as a Cray X40 supercomputer that consists of many nodes that are connected by a high speed network. For the compute nodes, each one 
...
In the README file, the correct program environment should be included for the codes to be successful. For compiling on Cray, there was no need to load
...
only have 12 tasks. When running this example on Magnus, no changes were made to the README, but there were minor changes to the SLURM.
...
The launching of the job by aprun was done by specifying that there was one node and 24 threads. The number of threads is also called a "depth" and
hence, OMP_NUM_THREADS was also set to 24 giving an expression of:

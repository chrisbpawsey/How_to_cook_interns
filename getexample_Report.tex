...
3) The example should be clear and completely detailed with steps to help the users understand and apply it.
...
1) Before introducing the examples to the users, it should be made sure that the examples run without encountering any errors. For example,
...
\begin{verbatim*}
\mbox{module swap PrgEnv-cray PrgEnv-<name of environment>} 
\end{verbatim*}
...
\mbox{SCRATCH=$MYSCRATCH/run_hostname/$SLURM_JOBID}
\mbox{RESULTS=$MYGROUP/mpifortran_cray_results/$SLURM_JOBID}
...
\begin{verbatim}
\mbox{module swap PrgEnv-cray PrgEnv-gnu}
...
(-02) is the optimization method, while -o placed after the source code directs to an executable called (hello_mpi_cray) which was previously defined in
the SLURM and chosen by the user as name. For GNU and Intel, this was called (hello_mpi_gnu and hello_mpi_intel). 
...
To compile the (hello_mpi.c) code, the following command was used: 
